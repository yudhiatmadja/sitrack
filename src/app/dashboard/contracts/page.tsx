    ...
    <div className="mb-4 space-y-4">
      ...
      <div className="relative">
        ...
        <input
          ...
          name="q"
          defaultValue={query}
          placeholder="Cari berdasarkan nomor kontrak atau nama site"
          className="block w-full max-w-md pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
        />
      </div>
...
export default async function ContractsPage({
  ...
  const canEdit =
    userRole === "Asset" || userRole === "SuperAdmin" || userRole === "Legal";
    ...
    if (error.code === "42501") {
      return (
        <p className="p-8 text-red-500">
          Error: Anda tidak memiliki izin untuk melihat data ini. (RLS Denied)
        </p>
      );
    }
    return (
  ...
  const getSiteName = (
    sites: ContractQueryResponse["sites"]
  ): string | null => {
  ...
  const filteredContracts = transformedContracts.filter((contract) => {
    ...
    if (query) {
      ...
      const siteMatch =
        contract.sites_name?.toLowerCase().includes(searchTerm) || false;
      const statusMatch =
        contract.status?.toLowerCase().includes(searchTerm) || false;
